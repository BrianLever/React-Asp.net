...
namespace ScreenDox.Server.Controllers
{
    public class ScreeningResultCreator
    {
        ...
        public ScreeningResultCreator(IScreeningResultService screeningResultService,
                                      IVisitService bhsVisitService,
        ...
        public bool SaveDemographicsResult(PatientDemographicsKioskResult result, ScreeningTimeLogRecord[] timeLog)
        {
            ...
            try
            {
                if (ValidateKiosk(result.KioskID))
                {
                    ...
                    demographicsModel.MilitaryExperience = result.MilitaryExperience.Select(x => new LookupValue { Id = x }).ToList();

                    var kioskUserAccount = _userService.GetUserByName("kiosk");
        ...
        private bool ValidateKiosk(short kioskID)
        {
            ...
            if (cert == null)
            {
                _errorLoggerService.Add(TextMessages.KioskEndpoint_ActivationLicenseErrorMesage, null, kioskID);
                succeed = false;
            }
