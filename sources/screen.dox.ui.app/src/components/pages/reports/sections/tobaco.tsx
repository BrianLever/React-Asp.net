...
const TobacoSection = (): React.ReactElement => {
    ...
    if(!reportListByAge['TobaccoSection']) {
      return <></>;
    }

    if((!reportListByAge['TobaccoSection']['Items'].length && !reportListByAge['TobaccoSection']['MainQuestions'].length)) {
      return <></>;
    }
    ...
    return (
      <TableContainer>
        ...
        <TableHead className={customClasss.tableHead}>
          <TableRow>
            <TableCell width={'50%'}><ReportHeaderText>{reportListByAge['TobaccoSection']['Header']}</ReportHeaderText></TableCell>
        ...
        <TableBody>
        {(typeof(reportListByAge['TobaccoSection']['MainQuestions']) !== "undefined") && reportListByAge['TobaccoSection']['MainQuestions'].length != 0 &&
          reportListByAge['TobaccoSection']['MainQuestions'].map((item: any) => 
            (<TableRow>
        ...
        {(typeof(reportListByAge['TobaccoSection']['Items']) !== 'undefined') && reportListByAge['TobaccoSection']['Items'].length !== 0 &&
          reportListByAge['TobaccoSection']['Items'].map((item: any) => 
            (<TableRow>
              <TableCell>
        ...
        <p style={{ marginTop: 10, marginBottom: 30 }}>{reportListByAge['TobaccoSection']['Copyrights']}</p>
      </TableContainer>
    )
}

export default TobacoSection;