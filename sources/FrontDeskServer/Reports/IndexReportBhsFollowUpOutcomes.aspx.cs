...
public partial class IndexReportBhsFollowUpOutcomesPage : IndicatorReportBasePage
{
    ...
    {
        Master.PageHeaderText = "Follow-Up Outcomes";
    ...
    protected override void PrintReport()
    {
        try
        {
            ...
            report.CreatePDF(HttpContext.Current, "IR_BhsFollowUpOutcomes.pdf");

        }
        catch (Exception ex)
        {
            SetErrorAlert(CustomError.GetInternalErrorMessage(), this.GetType());
            ErrorLog.AddServerException(CustomError.GetMessageForCustomOperation("print", "Follow-Up Outcomes"), ex);
        }
    }
