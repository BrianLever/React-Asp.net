...
public partial class IndexReportPatientDemographicsPage : IndicatorReportBasePage
{
    public bool IsUniquePatientMode
    {
        get
        {
            return true; // ucReportType.ReportType == UI.IndicatorReportTypeControl.IndicatorReportRenderType.UniquePatients;
        }
    }
    ...
    {
        Master.PageHeaderText = "Patient Demographics Report - Visits";
    ...
    protected override void PrintReport()
    {
        try
        {
            ...
            var report = new BhsDemopgraphicsVisitsPdfReport(filter, IsUniquePatientMode);
            report.CreatePDF(HttpContext.Current, "IR_PatientDemographics-BHSVisits.pdf");

        }
        catch (Exception ex)
        {
            SetErrorAlert(CustomError.GetInternalErrorMessage(), this.GetType());
            ErrorLog.AddServerException(CustomError.GetMessageForCustomOperation("print", "Patient Demographics Report - Visits"), ex);
        }
    }
