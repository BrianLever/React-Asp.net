...
public partial class IndexReportByAge : IndicatorReportBasePage
{
    ...
    {
        Master.PageHeaderText = "Behavioral Health Indicator Report Screening Results by Age";
    ...
    protected override void PrintReport()
    {
        try
        {
            ...
            var report = new BhsIndicatorByAgePdfReport(filter, IsUniquePatientMode);
            report.CreatePDF(HttpContext.Current, "IR_ScreeningResultsByAge.pdf");

        }
        catch (Exception ex)
        {
            SetErrorAlert(CustomError.GetInternalErrorMessage(), this.GetType());
            ErrorLog.AddServerException(CustomError.GetMessageForCustomOperation("print", "Indicator Report By Age"), ex);
        }
    }
