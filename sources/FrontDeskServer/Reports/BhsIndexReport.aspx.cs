...
public partial class BhsIndexReport : IndicatorReportBasePage
{
    ...
    {
        Master.PageHeaderText = "Behavioral Health Indicator Report Screening Results by Problem";
    ...
    protected override void PrintReport()
    {
        try
        {
            ...
            DateTime? start = !dtrDateFilter.StartDate.HasValue ? (DateTime?)null : dtrDateFilter.StartDate;
            DateTime? end = !dtrDateFilter.EndDate.HasValue ? (DateTime?)null : dtrDateFilter.EndDate;
            ...
            BhsIndicatorPdfReport report = new BhsIndicatorPdfReport(filter, IsUniquePatientMode);
            report.CreatePDF(HttpContext.Current, "IR_ScreeningResultsByProblem.pdf");

        }
        catch (Exception ex)
        {
            SetErrorAlert(CustomError.GetInternalErrorMessage(), this.GetType());
            ErrorLog.AddServerException(CustomError.GetMessageForCustomOperation("print", "Indicator Report"), ex);
        }
    }
