...
public partial class IndexReportBhsVisitOutcomesPage : IndicatorReportBasePage
{
    ...
    {
        Master.PageHeaderText = "Visit Outcomes";
    ...
    protected override void PrintReport()
    {
        try
        {
            ...
            var report = new BhsVisitOutcomesPdfReport(filter, IsUniquePatientMode);
            report.CreatePDF(HttpContext.Current, "IR_BhsVisitsOutcomes.pdf");

        }
        catch (Exception ex)
        {
            SetErrorAlert(CustomError.GetInternalErrorMessage(), this.GetType());
            ErrorLog.AddServerException(CustomError.GetMessageForCustomOperation("print", "Visit Outcomes"), ex);
        }
    }
