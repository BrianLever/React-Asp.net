...
public partial class ScreenTimeLogPage : IndicatorReportBasePage
{
    public bool IsUniquePatientMode
    {
        get
        {
            return false; // return ucReportType.ReportType == UI.IndicatorReportTypeControl.IndicatorReportRenderType.UniquePatients;
        }
    }
    ...
    {
        Master.PageHeaderText = "Screen Time Log";
    ...
    protected override void PrintReport()
    {
        try
        {
            ...
            var report = new ScreenTimeLogPdfReport(filter);
            report.CreatePDF(HttpContext.Current, "IR_ScreenTimeLog.pdf");

        }
        catch (Exception ex)
        {
            SetErrorAlert(CustomError.GetInternalErrorMessage(), this.GetType());
            ErrorLog.AddServerException(CustomError.GetMessageForCustomOperation("print", "Screen Time Log"), ex);
        }
    }
