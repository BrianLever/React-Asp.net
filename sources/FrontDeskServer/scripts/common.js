...
function configLogSource(flag, enabled) {
    appConfig.logging[flag] = enabled;
}

function isLogEnabled(flag) {
    return appConfig.logging[flag];
}

function logDebug(text, flag) {

    if (appConfig.logging[flag]) {
        console.debug("[" + flag + "] " + text);
    }
}

function logInfo(text, flag) {
    if (appConfig.logging[flag]) {
        console.info("[" + flag + "] " + text);
    }
}
...
var windowResizeLogSource = "resize window";
configLogSource(windowResizeLogSource, false);
...
function onResize() {
    ...
    try {
        ...
        if (isLogEnabled(windowResizeLogSource)) {
            logDebug("resizing. iHeight = " + iHeight, windowResizeLogSource);
        }

        $(footerDiv).css("visibility", "visible");


        var login = document.getElementById("login-outer");
        if (login != null) {
            $("#content")
                .css("display", "table")
                .css("width", "100%");
            ...
            if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1) {
                $("#content")
                    .css("height", (iHeight) + "px")
                    .css("min-height", (iHeight) + "px");
            }

            $("#content #pageHeader").css("display", "none");
            $("#login-outer").css("visibility", "visible").css("padding", "0");
        }
...
function BrowserWindow() {       // Function to be used for getting the dimensions of browser window.
    ...
    if (document.documentElement && (document.documentElement.clientWidth || document.documentElement.clientHeight)) {
        ...
        if (isLogEnabled(windowResizeLogSource)) {
            logDebug("non-ie browser resize.IE 6+ in 'standards compliant mode'", windowResizeLogSource);
        }
        ...
        if (isLogEnabled(windowResizeLogSource)) {
            logDebug("iClientWidth: " + iClientWidth + ", iClientHeight: " + iClientHeight, windowResizeLogSource);
        }
    }
    if (typeof (window.innerWidth) == 'number') {
        //Non-IE
        iClientWidth = Math.max(window.innerWidth, iClientWidth);
        iClientHeight = Math.max(window.innerHeight, iClientHeight);


        logDebug("[resize window] non-ie browser resize.");
        logDebug("[resize window] iClientWidth: " + iClientWidth + ", iClientHeight: " + iClientHeight);
        ...
        if (isLogEnabled(windowResizeLogSource)) {
            logDebug("IE 4 compatible resize", windowResizeLogSource);
            logDebug("iClientWidth: " + iClientWidth + ", iClientHeight: " + iClientHeight, windowResizeLogSource);
        }
    }
