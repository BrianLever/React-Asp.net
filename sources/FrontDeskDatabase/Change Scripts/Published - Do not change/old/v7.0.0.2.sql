IF EXISTS(select 1 from sys.columns where OBJECT_ID = OBJECT_ID('dbo.ScreeningSectionAge') AND name = 'AgeIsNotConfigurable')
SET NOEXEC ON
GO

ALTER TABLE dbo.ScreeningSectionAge ADD AgeIsNotConfigurable bit NULL;

GO

UPDATE dbo.ScreeningSectionAge SET AgeIsNotConfigurable = 0;

ALTER TABLE dbo.ScreeningSectionAge ALTER COLUMN AgeIsNotConfigurable bit NOT NULL;
ALTER TABLE dbo.ScreeningSectionAge ADD CONSTRAINT  DF__ScreeningSectionAge_AgeIsNotConfigurable DEFAULT 0  FOR AgeIsNotConfigurable;

SET NOEXEC OFF
GO

UPDATE dbo.ScreeningSectionAge SET LastModifiedDateUTC = GETUTCDATE();
...
GO
---------------------------------------------
IF NOT EXISTS(SELECT 1 FROM dbo.DbVersion WHERE DbVersion =  '7.0.0.2')
INSERT INTO dbo.DbVersion(DbVersion) VALUES('7.0.0.2');